...
class TestVersionManager(unittest.TestCase):
    ...
    def test_get_current_version_single_version_found(self):
        ...
        version_doc = {"collection_name": self.collection_name, "current_version": f"{self.collection_name}.1.2.3.4"}
    ...
    def test_get_current_version_multiple_versions_found(self):
        ...
        version_docs = [
            {"collection_name": self.collection_name, "current_version": f"{self.collection_name}.1.2.3.4"},
            {"collection_name": self.collection_name, "current_version": f"{self.collection_name}.1.2.3.5"}
        ]
    ...
    def test_update_version_new_version(self):
        ...
        mock_version_doc = {"collection_name": self.collection_name, "current_version": f"{self.collection_name}.1.2.3.4"}
        self.mock_mongo_io.upsert.return_value = mock_version_doc
        
        # Mock get_current_version to return the updated version
        self.mock_mongo_io.get_documents.return_value = [{"current_version": f"{self.collection_name}.1.2.3.4"}]
        
        result = VersionManager.update_version(self.mock_mongo_io, self.collection_name, f"{self.collection_name}.1.2.3.4")
        ...
        self.mock_mongo_io.upsert.assert_called_once_with(
            self.version_collection_name,
            match={"collection_name": self.collection_name},
            data={"collection_name": self.collection_name, "current_version": f"{self.collection_name}.1.2.3.4"}
        )

    def test_update_version_existing_version(self):
        """Test update_version for an existing version"""
        # Mock upsert to return a document
        mock_version_doc = {"collection_name": self.collection_name, "current_version": f"{self.collection_name}.2.0.0.1"}
        self.mock_mongo_io.upsert.return_value = mock_version_doc
        
        # Mock get_current_version to return the updated version
        self.mock_mongo_io.get_documents.return_value = [{"current_version": f"{self.collection_name}.2.0.0.1"}]
        
        result = VersionManager.update_version(self.mock_mongo_io, self.collection_name, f"{self.collection_name}.2.0.0.1")
        ...
        self.mock_mongo_io.upsert.assert_called_once_with(
            self.version_collection_name,
            match={"collection_name": self.collection_name},
            data={"collection_name": self.collection_name, "current_version": f"{self.collection_name}.2.0.0.1"}
        )
    ...
    def test_update_version_invalid_version_format(self):
        ...
        with self.assertRaises(ConfiguratorException):
            VersionManager.update_version(self.mock_mongo_io, self.collection_name, f"{self.collection_name}.invalid.version")
    ...
    def test_version_lifecycle(self):
        ...
        self.mock_mongo_io.upsert.return_value = {"collection_name": self.collection_name, "current_version": f"{self.collection_name}.1.0.0.1"}
        self.mock_mongo_io.get_documents.return_value = [{"current_version": f"{self.collection_name}.1.0.0.1"}]
        
        updated_version = VersionManager.update_version(self.mock_mongo_io, self.collection_name, f"{self.collection_name}.1.0.0.1")
